...
const AFFECTIVE_FIELDS = [
  {
    key: "punctuality",
    label: "Punctuality",
    description: "Reports to school and activities on time.",
  },
  {
    key: "attentiveness",
    label: "Attentiveness",
    description: "Pays attention during lessons and school activities.",
  },
  {
    key: "neatness",
    label: "Neatness",
    description: "Maintains personal and academic cleanliness.",
  },
  {
    key: "politeness",
    label: "Politeness",
    description: "Shows courtesy and respect to others.",
  },
  {
    key: "reliability",
    label: "Reliability",
    description: "Can be trusted to carry out assigned duties.",
  },
  {
    key: "honesty",
    label: "Honesty",
    description: "Demonstrates truthfulness and integrity.",
  },
  {
    key: "initiative",
    label: "Initiative",
    description: "Shows willingness to act and take positive initiative.",
  },
  {
    key: "attitudeToWork",
    label: "Attitude to Work",
    description: "Demonstrates a positive attitude towards school work.",
  },
];

function ResultAffectiveDomain({
  selectedStudent,
  academicSession,
  term,
  affectiveDomain,
  onAffectiveDomainChange,
  onBack,
  onContinue,
  error,
}) {
  ...
  */
  ...
  const validation = useMemo(() => {
    const incomplete = AFFECTIVE_FIELDS.find(
      (field) => affectiveDomain[field.key] === "",
    );
    ...
    const invalid = AFFECTIVE_FIELDS.find((field) => {
      const value = Number(affectiveDomain[field.key]);
  ...
  */
  ...
  const totalScore = useMemo(() => {
    return AFFECTIVE_FIELDS.reduce(
      (total, field) => total + (Number(affectiveDomain[field.key]) || 0),
      0,
    );
  }, [affectiveDomain]);
  ...
  */
  ...
  const averageScore = useMemo(() => {
    if (!AFFECTIVE_FIELDS.length) {
      return 0;
    }

    return (totalScore / AFFECTIVE_FIELDS.length).toFixed(2);
  }, [totalScore]);
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-6">
          <div className="mb-2 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Affective Domain</h1>

              <p className="text-sm text-slate-400">
                Rate the student's behavioural and interpersonal
                characteristics.
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-white">Affective</span>
            </div>

            <div className="hidden h-px flex-1 bg-slate-800 md:block" />
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 shadow-xl">
          ...
          <div className="border-b border-white/10 px-5 py-4">
            <h2 className="text-lg font-semibold">Behavioural Assessment</h2>
          ...
          <div className="divide-y divide-white/5">
            {AFFECTIVE_FIELDS.map((field) => {
              const value = affectiveDomain[field.key];
            ...
            >
              {validation.valid ? (
                <div className="flex items-center gap-2">
                  ...
                  <span>All affective-domain ratings have been completed.</span>
                </div>
              ) : (
                validation.message
              )}
...
export default ResultAffectiveDomain;
